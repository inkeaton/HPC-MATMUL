...
int main(int argc, char **argv)
{
     ...
     */
    double (* restrict a)[N] = aligned_alloc(BOUNDARY, sizeof(double[N][N]));
    double (* restrict b)[N] = aligned_alloc(BOUNDARY, sizeof(double[N][N]));
    double (* restrict c)[N] = aligned_alloc(BOUNDARY, sizeof(double[N][N]));
            ...
            for (int jj = 0; jj < N; jj += BLOCK_SIZE) {
                ...
                for (int i = ii; i < i_max; i++) {
                    for (int k = kk; k < k_max; k++) {
                         ...
                         */
                        #pragma omp simd
                        // #pragma code_align(32) // I believe xHost already does this, let's ignore it
    ...
    // for (int ii = 0; ii < N; ii += BLOCK_SIZE) {
    //     for (int jj = 0; jj < N; jj += BLOCK_SIZE) { // Swap jj and kk
    //         for (int kk = 0; kk < N; kk += BLOCK_SIZE) {
    ...
    /* Timing the end and reporting */
        ...
        fprintf(stderr, "[seq-tile] N=%d, block=%d, elapsed=%.3f s\n", 
